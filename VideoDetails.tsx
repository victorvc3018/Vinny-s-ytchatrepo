...
const VideoDetails: React.FC<VideoDetailsProps> = ({ videoInfo, isLoading }) => {
  return (
    <div className="mt-4 text-white">
      {isLoading ? (
        <SkeletonText className="h-7 w-3/4 mb-1" />
      ) : (
        <h1 className="text-xl font-bold">{videoInfo?.title || 'Video Title Not Found'}</h1>
      )}
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            {isLoading ? (
              ...
              <>
                <p className="font-semibold">{videoInfo?.author_name || 'Channel Name'}</p>
      ...
      <div className="bg-[#272727] p-3 rounded-lg mt-4 text-sm hover:bg-gray-800 transition-colors cursor-pointer">
        <p className="font-bold">1.2M views &nbsp; 3 days ago</p>
        <p className="mt-2">
           {isLoading ? (
